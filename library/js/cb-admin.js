(function( $ ){
         $.fn.cbSectionCode = function(cb_current_section, cb_block_type, cbFilterCheck ) {
          ...
          $('#tax_meta_class_nonce').before('<h2 id="cb-header">Valenti Options</h2><h3 id="cb-subhead">General Options</h3>');
        $('#nonce-delete-mupload_cb_bg_image_field_id').closest('.form-field').before('<h3 id="cb-subhead">Valenti Background Options</h3>');
      })( jQuery );
...
jQuery(document).ready(function($){"use strict";
    ...
    var cbReviewCheckbox = $('#cb_review_checkbox'),
      ...
      cbHpb.find('.option-tree-ui-radio-image-selected[title="Custom Code 50% Width"]').closest('.ui-state-default').addClass('cb-half-width');
      ...
      cbSectionA.add(cbSectionB).add(cbSectionC).add(cbSectionD).on('click', '.option-tree-ui-radio-image', function() {
          ...
          cbCurrentBlock.cbSectionCode(cbCurrentSectionID, cbCurrentModule);
           ...
           if ( ( cbCurrentModuleTitle === 'Ad: 336x280') || ( cbCurrentModuleTitle === 'Module B') || ( cbCurrentModuleTitle === 'Custom Code 50% Width') || ( cbCurrentModuleTitle === 'Module C') || ( cbCurrentModuleTitle === 'Module D') )  {
...
jQuery('select[id^="widget-cb-popular-posts"]').on('change', function() {
    ...
    var cbInput = jQuery(this),
        cbInputChange = cbInput.val(),
        cbDateFilter = cbInput.closest('p').next();

    if ( cbInputChange == 'cb-visits' ) {
